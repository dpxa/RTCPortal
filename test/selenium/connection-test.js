...
async function runConnectionTest() {
  ...
  try {
    ...
    console.log(
      `Peer 1 'Copy My ID' button visible: ${await copyIdButton1.isDisplayed()}`
    );
    ...
    console.log(
      `Peer 2 'Copy My ID' button visible: ${await copyIdButton2.isDisplayed()}`
    );
    ...
    console.log(
      `Peer 1 'Connect' button initial enabled state: ${await connectButton1.isEnabled()}`
    );
    await partnerIdField1.sendKeys(peer2Id);
    console.log(
      `Peer 1 entered Peer 2's ID (${peer2Id}) into partner ID field.`
    );

    await driver1.wait(until.elementIsEnabled(connectButton1), 20000);
    console.log(
      `Peer 1 'Connect' button enabled after entering Peer 2's ID: ${await connectButton1.isEnabled()}`
    );
    ...
    console.log(
      `Peer 1 'Active Connection Container' visible: ${await activeConnectionContainer1.isDisplayed()}`
    );

    await driver1.wait(
      until.elementTextContains(activeConnectionStatus1, peer2Id),
      30000
    );
    console.log(
      `Peer 1 'Active Connection Status' shows Peer 2's ID: ${await activeConnectionStatus1.getText()}`
    );
    ...
    console.log(
      `Peer 1 'End' button visible and text is 'DISCONNECT': ${await endButton1.isDisplayed()} | ${await endButton1.getText()}`
    );

    await driver1.wait(until.elementIsVisible(fileTransferSection1), 20000);
    console.log(
      `Peer 1 'File Transfer Section' visible: ${await fileTransferSection1.isDisplayed()}`
    );
    ...
    console.log(
      `Peer 2 'Active Connection Container' visible: ${await activeConnectionContainer2.isDisplayed()}`
    );

    await driver2.wait(
      until.elementTextContains(activeConnectionStatus2, peer1Id),
      30000
    );
    console.log(
      `Peer 2 'Active Connection Status' shows Peer 1's ID: ${await activeConnectionStatus2.getText()}`
    );
    ...
    console.log(
      `Peer 2 'End' button visible and text is 'DISCONNECT': ${await endButton2.isDisplayed()} | ${await endButton2.getText()}`
    );

    await driver2.wait(until.elementIsVisible(fileTransferSection2), 20000);
    console.log(
      `Peer 2 'File Transfer Section' visible: ${await fileTransferSection2.isDisplayed()}`
    );
    ...
    console.log(
      `Peer 1 - Found GitHub link with text: '${await githubLink1.getText()}' and href: ${await githubLink1.getAttribute(
        "href"
      )}`
    );
